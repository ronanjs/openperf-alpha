 ...
 * @Last Modified time: 2020-10-26 09:23:38
 */
...
if (process.argv.length === 3) {
  const gen = OpenPerfNode.newGenerator(process.argv[2], 'port0')
  const ana = OpenPerfNode.newAnalyser(process.argv[2], 'port1')
  testOneWayLatency(gen, ana)
} else if (process.argv.length === 4) {
  const gen = OpenPerfNode.newGenerator(process.argv[2], 'port0', { timesource: '192.168.10.97' })
