 ...
 * @Last Modified time: 2020-10-26 08:37:07
 */
...
const {
  OpenPerfClient
} = require('./core/opclient')
const {
  TimeSources
} = require('./core/timesource')
...
async function run (genServerIP, genPortID, anaServerIP, anaPortID) {
  ...
  const genTimesource = await genTimesources.create('timesource-01', '192.168.10.97')

  const anaTimesources = new TimeSources(anaClient)
  const anaTimesource = await anaTimesources.create('timesource-01', '192.168.10.98')
